...
// Module xử lý video capture từ thiết bị.
// Bao gồm log debug chi tiết và xử lý deviceId rỗng.
...
function captureFrame() {
    ...
    try {
        ...
        const dataUrl = tempCanvas.toDataURL('image/png'); // Luôn capture dạng PNG để đảm bảo chất lượng
        ...
        if (typeof onCaptureSuccessCallback === 'function') {
            ...
            onCaptureSuccessCallback(dataUrl, filename); // Truyền cả dataUrl và filename
        } else {
 ...
 * @param {string} options.sourceSelectorId - ID của thẻ <select> chọn nguồn
 * @param {string} options.startButtonId - ID của nút bắt đầu/chọn nguồn
 * @param {function(string, string)} options.onCaptureSuccess - Callback khi chụp ảnh thành công (nhận dataUrl, filename).
 */
export function initVideoCapture(options) {
    ...
    startButton.addEventListener('click', () => {
        ...
        if (!selectedDeviceId) {
            console.log("[VideoCapture] DEBUG: No specific device selected. Attempting generic video request...");
            startVideoStream(null, options.sourceSelectorId); // Yêu cầu camera mặc định
            ...
            startVideoStream(selectedDeviceId, options.sourceSelectorId); // Yêu cầu camera cụ thể
        }
        // ---------------------------------
    });
