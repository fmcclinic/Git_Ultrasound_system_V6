...
// js/modules/image-handler.js (Updated for OCR + Video Capture Integration)
// ... (Toàn bộ code initImageHandler, handleImageFiles, removeImage, getUploadedImageData, getImageDataById, clearImageData giữ nguyên như phiên bản trước) ...

/**
 * NEW: Adds an image from a Data URL to the preview and internal list.
 * Used for images captured from video stream or potentially other sources.
...
export function addImageFromDataUrl(dataUrl, filename) {
    ...
    // Extract mime type and approximate size (size calculation from base64 is rough)
    const mimeMatch = dataUrl.match(/data:(image\/\w+);/);
    const mimeType = mimeMatch ? mimeMatch[1] : 'image/png'; // Default to png
    const base64Data = dataUrl.split(',')[1];
    // Estimate size: Base64 is ~4/3 larger than original binary size
    const approxSize = Math.ceil(base64Data.length * 0.75);

    const fileInfo = {
        id: `img_${Date.now()}_${Math.random().toString(16).slice(2)}`,
        name: filename,
        size: approxSize, // Approximate size
        type: mimeType,
        dataUrl: dataUrl // Store the full Data URL
    };
    ...
    // Create DOM Elements for preview (Similar to handleImageFiles)
    try {
        const wrapper = document.createElement('div');
        wrapper.className = 'image-preview-wrapper';
        ...
        const removeBtn = document.createElement('button');
        removeBtn.textContent = 'X';
        removeBtn.className = 'remove-image-btn danger-btn';
        ...
        // onclick listener added by obstetric-module.js if OCR is available
        wrapper.appendChild(ocrBtn);
...
console.log("image-handler.js loaded (with OCR button & addImageFromDataUrl support).");