// js/core/report-core.js (ADAPTED for Obstetric Project - v1 Single Column)
// Handles report generation, preview, translation, editing, printing, and exporting for OBSTETRIC reports.
...
const defaultReportTitleEN = "OBSTETRIC ULTRASOUND REPORT"; // <-- CHANGED
const defaultReportTitleVI = "BÁO CÁO SIÊU ÂM SẢN KHOA"; // <-- CHANGED
...
function formatReportAsHtml(patientData, organData, images, reportTitle = defaultReportTitleEN) {
    console.log("[ReportCore - Obstetric] Formatting English report as HTML...");
    if (!organData || typeof organData.formatReportSectionHtml !== 'function') {
        console.error("[ReportCore - Obstetric] Invalid organData or missing formatReportSectionHtml method.");
    ...
    report += `<div class="report-section report-findings-section">
                ...
                ${ organData.formatReportSectionHtml() } </div>`;
...
function generateVietnameseReportHtmlForPrint(patientData, translatedBodyHtml, images) {
    console.log("[ReportCore - Obstetric] Generating Vietnamese HTML report structure for printing...");
    ...
    report += `</div>`;
    console.log("[ReportCore - Obstetric] Vietnamese HTML for print generated.");
    return report;
}

/**
 * Opens a print window with the provided HTML content and styles.
 * UPDATED embedded CSS to use single-column layout for OB findings.
 */
function printPreparedHtml(htmlContent, title = 'Print Report') {
    try {
        ...
        printWindow.document.write(`
            ...
            <style>
                /* === Embedded Print Styles (Obstetric - v1 Single Column) === */
                ...
                /* --- START: Obstetric Findings Section Print Styles (Single Column) --- */
                .report-findings-section { page-break-inside: auto; }
                .report-findings-section .obstetric-findings-container {
                    margin-top: 3mm !important;
                    page-break-inside: auto !important; /* Allow findings to break across pages */
                }
                /* Titles within the single column */
                .report-findings-section .obstetric-findings-container h4 { /* e.g., Biometry, Anatomy... */
                ...
                .report-findings-section .obstetric-findings-container h4:first-child { margin-top: 0;}
                .report-findings-section .obstetric-findings-container h5 { /* e.g., Head/Brain... */
                ...
                .report-findings-section .obstetric-findings-container > h4 + p {
                    page-break-inside: auto; /* Allow breaking within long paragraphs */
                }
                /* --- END: Obstetric Findings Section Print Styles --- */
 ...
 * UPDATED embedded CSS for single-column findings.
 */
function exportReportAsHtml(reportHtml, filename = 'obstetric-ultrasound-report.html') { // <-- CHANGED default filename
    // === UPDATED Embedded Styles to match printPreparedHtml (Obstetric Single Column) ===
    const printStyles = `
        @page { size: A4; margin: 12mm; }
        ...
        /* Obstetric Findings Styles (Single Column) */
        .report-findings-section .obstetric-findings-container { margin-top: 3mm !important; }
        .report-findings-section .obstetric-findings-container h4 { font-size: 1.05em; font-weight: bold; color: #000; margin-top: 0.6em; margin-bottom: 0.3em; padding-bottom: 1px; border-bottom: 1px dotted #aaa; }
        .report-findings-section .obstetric-findings-container h4:first-child { margin-top: 0;}
        .report-findings-section .obstetric-findings-container h5 { font-size: 1em; font-weight: bold; color: #333; margin-top: 0.5em; margin-bottom: 0.2em; border-bottom: none; padding-bottom: 0; padding-left: 5mm;}
        .report-findings-section .obstetric-findings-container ul { list-style: none; padding-left: 5mm; margin-top: 0.2em; margin-bottom: 0.8em; }
        .report-findings-section .obstetric-findings-container li { margin-bottom: 0.3em; line-height: 1.4; }
        .report-findings-section .obstetric-findings-container li strong { color: #000; margin-right: 4px; display: inline-block; min-width: 100px; }
        .report-findings-section .obstetric-findings-container > hr + h4, .report-findings-section .obstetric-findings-container > h4:last-of-type { margin-top: 8mm; padding-top: 4mm; border-top: 1px solid #ccc; border-bottom: none; font-size: 1.05em; padding-left: 0;}
        /* General layout */
    ...
    console.log(`[ReportCore - Obstetric] Report exported as ${filename}`);
}
...
export function initReportSystem() {
    ...
    for (const key in requiredElements) {
        if (!requiredElements[key]) {
            console.error(`[ReportCore - Obstetric] Initialization failed: Element with ID matching '${key}' not found.`);
            showNotification(`Report UI element missing: ${key}. Check HTML.`, "error");
            return; // Stop initialization if UI is broken
        }
    }
    console.log("[ReportCore - Obstetric] All required report UI elements found.");

    // --- Generate Report Button Listener ---
    generateBtn.addEventListener('click', async () => {
        // === UPDATED Log ===
        console.log("[ReportCore - Obstetric] Generate Report button clicked.");
        // Reset UI
        ...
        try {
            ...
            if (!currentPatientData.reportingDoctor) {
                console.warn("[ReportCore - Obstetric] Reporting doctor name is empty.");
                // Optionally provide a default or prompt? For now, just warn.
            }

            // === UPDATED: Dynamically import and call OB data collection ===
            const { collectObstetricData } = await import('../organs/obstetric/obstetric-module.js'); // <-- CHANGE
            const organData = collectObstetricData(); // <-- CHANGE
            if (!organData || typeof organData.formatReportSectionHtml !== 'function') {
                 throw new Error("Failed to collect or format Obstetric assessment data."); // <-- CHANGE
            }

            const images = getUploadedImageData(true); // Get image data
            const reportTitle = defaultReportTitleEN; // Use OB default title
            generatedReportHtml = formatReportAsHtml(currentPatientData, organData, images, reportTitle); // <-- Uses OB title
            // === UPDATED Log ===
            console.log("[ReportCore - Obstetric] Report HTML generated.");

            // Display Report & Show Buttons
            reportPreview.innerHTML = generatedReportHtml;
            // === UPDATED Log ===
            console.log("[ReportCore - Obstetric] Report preview updated.");
            ...
            if (getApiKey() && getTranslationPrompt()) {
                ...
                // === UPDATED Log ===
                console.warn("[ReportCore - Obstetric] Translate button hidden: API Key or Prompt missing.");
                showNotification("Translation disabled: Config missing.", "info");
            }
            // === UPDATED Notification ===
            showNotification("Obstetric report generated successfully!", "success");

        } catch (error) {
            // === UPDATED Log ===
            console.error("[ReportCore - Obstetric] Error during report generation:", error);
    ...
    translateBtn.addEventListener('click', async () => {
        // === UPDATED Log ===
        console.log("[ReportCore - Obstetric] Translate Report button clicked.");
        ...
        try {
             ...
             const findingsContainer = tempDiv.querySelector('.report-findings-section .obstetric-findings-container'); // <-- CHANGE
             if (!findingsContainer) {
                throw new Error("Could not find the findings container (.obstetric-findings-container) in the generated HTML.");
             }
             ...
             console.error("[ReportCore - Obstetric] Error extracting text:", extractError);
        ...
        try {
            ...
            console.error("[ReportCore - Obstetric] Translation failed:", error);
    ...
    printBtn.addEventListener('click', () => {
        if (generatedReportHtml) {
            // === UPDATED Title ===
            const title = `OB US Report - ${currentPatientData?.patientId || 'Unknown'} - ${currentPatientData?.examDate || ''}`;
            printPreparedHtml(generatedReportHtml, title); // Uses the updated function with OB CSS
    ...
    printTranslationBtn.addEventListener('click', () => {
        if (currentTranslationMarkdown) {
             ...
             // === UPDATED Title ===
             const titleVI = `Báo cáo SA Sản Khoa - ${currentPatientData?.patientId || 'Unknown'} - ${currentPatientData?.examDate || ''}`;
             printPreparedHtml(vietnameseReportHtml, titleVI); // Use the same print function with OB CSS
    ...
    exportBtn.addEventListener('click', () => {
         if (generatedReportHtml) {
             ...
             // === UPDATED Filename ===
             const filename = `OB_US_Report_${patientId}_${examDate}.html`;
...
// === UPDATED Log ===
console.log("report-core.js (Obstetric Version - v1 Single Column) loaded.");