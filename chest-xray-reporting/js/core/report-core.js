// js/core/report-core.js (ADAPTED for Chest X-ray Project - v1 Single Column)
// Handles report generation, preview, translation, editing, printing, and exporting for CHEST X-RAY reports.

import { getFormData } from './form-core.js';
import { getUploadedImageData } from '../modules/image-handler.js'; // Generic image handler
import { showNotification } from './ui-core.js';
import { translateReportViaApi } from '../modules/report-translator.js'; // Generic translator
...
// === UPDATED ===
const defaultReportTitleEN = "CHEST X-RAY REPORT";
const defaultReportTitleVI = "BÁO CÁO X QUANG NGỰC";
 ...
 */
function formatReportAsHtml(patientData, organData, images, reportTitle = defaultReportTitleEN) { // <-- Use CXR default
    // === UPDATED ===
    console.log("[ReportCore - ChestXray] Formatting English report as HTML...");
    if (!organData || typeof organData.formatReportSectionHtml !== 'function') {
        // === UPDATED ===
        console.error("[ReportCore - ChestXray] Invalid organData or missing formatReportSectionHtml method.");
    ...
    // Patient Info (Structure remains the same, labels are bilingual in HTML)
    ...
    <p><strong>Exam Date / Ngày chụp:</strong> ${patientData.examDate || 'N/A'}</p> <p><strong>Requesting Dr / BS Chỉ định:</strong> ${patientData.requestingPhysician || 'N/A'}</p>
    <p><strong>Reason for Exam / Lý do khám:</strong> ${patientData.reason ? String(patientData.reason).replace(/\n/g, '<br>') : 'N/A'}</p>
    </div><hr class="report-hr">`;
    // Findings section (Delegated to CXR module)
    report += `<div class="report-section report-findings-section">
                <h3 class="report-section-title">FINDINGS / KẾT QUẢ</h3> ${ organData.formatReportSectionHtml() } </div>`; // Module generates its own specific container class inside this
    ...
    if (validImages.length > 0) {
        ...
        validImages.forEach((img, index) => {
            report += `<div class="report-image-item">
                <img src="${img.dataUrl}" alt="X-ray Image ${index + 1}" title="${img.name || `Image ${index + 1}`}" style="object-fit: contain; max-width: 100%; max-height: 100%; display: block; margin: auto;">
                <p class="caption">${img.name || `Image ${index + 1}`}</p></div>`; // Updated Alt text
        });
        report += `</div></div>`;
    }
    // Footer/Signature (Remains the same structure, updated name placeholder)
    report += `<hr class="report-hr"><div class="report-footer signature-section">
        <p class="signature-title">Bác sĩ Chẩn đoán hình ảnh</p> <div class="signature-space"></div> <p class="signature-name">${patientData.reportingDoctor || '[ Họ tên bác sĩ đọc KQ ]'}</p> </div>`;
    report += `</div>`;
    return report;
}
...
function generateVietnameseReportHtmlForPrint(patientData, translatedBodyHtml, images) {
    // === UPDATED ===
    console.log("[ReportCore - ChestXray] Generating Vietnamese HTML report structure for printing...");
    const reportTitleVI = defaultReportTitleVI; // Use CXR constant
    let report = `<div class="report-container">`;
    ...
    report += `<div class="report-section">
                ...
                <p><strong>Ngày chụp:</strong> ${patientData.examDate || 'N/A'}</p> <p><strong>BS Chỉ định:</strong> ${patientData.requestingPhysician || 'N/A'}</p>
    ...
    if (validImages.length > 0) {
        ...
        validImages.forEach((img, index) => {
            report += `<div class="report-image-item">
                <img src="${img.dataUrl}" alt="Hình X quang ${index + 1}" title="${img.name || `Hình ${index + 1}`}" style="object-fit: contain; max-width: 100%; max-height: 100%; display: block; margin: auto;"> <p class="caption">${img.name || `Hình ${index + 1}`}</p></div>`;
        });
        report += `</div></div>`;
    }
    // Footer/Signature (same structure, updated title)
    report += `<hr class="report-hr"><div class="report-footer signature-section">
        <p class="signature-title">Bác sĩ Chẩn đoán hình ảnh</p> <div class="signature-space"></div>
        <p class="signature-name">${patientData.reportingDoctor || '[ Họ tên bác sĩ đọc KQ ]'}</p> </div>`;
    report += `</div>`;
    // === UPDATED ===
    console.log("[ReportCore - ChestXray] Vietnamese HTML for print generated.");
    return report;
}

/**
 * Opens a print window with the provided HTML content and styles.
 * UPDATED embedded CSS to use single-column layout targeting `.cxr-findings-container`.
 */
function printPreparedHtml(htmlContent, title = 'Print Report') {
    try {
        ...
        printWindow.document.write(`
            ...
            <style>
                /* === Embedded Print Styles (ChestXray - v1 Single Column) === */
                ...
                /* --- START: ChestXray Findings Section Print Styles (Single Column) --- */
                /* === UPDATED SELECTOR === */
                .report-findings-section { page-break-inside: auto; }
                .report-findings-section .cxr-findings-container {
                    margin-top: 3mm !important;
                    page-break-inside: auto !important; /* Allow findings to break across pages */
                }
                /* Titles within the single column */
                .report-findings-section .cxr-findings-container h4 { /* e.g., Technical Factors, Findings, Comparison */
                ...
                .report-findings-section .cxr-findings-container h4:first-child { margin-top: 0;}
                .report-findings-section .cxr-findings-container h5 { /* e.g., Airways, Bones... */
                ...
                /* List items (e.g., for technical factors) */
                .report-findings-section .cxr-findings-container ul {
                    list-style: none; padding-left: 5mm; margin-top: 0.2em; margin-bottom: 0.8em; page-break-inside: avoid;
                }
                .report-findings-section .cxr-findings-container li {
                    margin-bottom: 0.3em; line-height: 1.4; page-break-inside: avoid; /* Avoid breaking mid-item */
                }
                .report-findings-section .cxr-findings-container li strong {
                    color: #000; margin-right: 4px; display: inline-block; min-width: 100px; /* Adjust if needed */
                }
                /* Paragraphs for descriptions */
                .report-findings-section .cxr-findings-container p {
                    margin-left: 5mm; /* Indent descriptions slightly */
                    margin-top: 0.2em;
                    margin-bottom: 0.6em;
                    page-break-inside: auto; /* Allow paragraphs to break */
                }
                /* Impression/Recommendation in Print */
                .report-findings-section .cxr-findings-container > hr + h4,
                .report-findings-section .cxr-findings-container > h4:last-of-type {
                ...
                .report-findings-section .cxr-findings-container > h4 + p {
                    page-break-inside: auto; /* Allow breaking within long paragraphs */
                }
                /* --- END: ChestXray Findings Section Print Styles --- */
 ...
 * UPDATED embedded CSS to target `.cxr-findings-container`.
 */
// === UPDATED default filename ===
function exportReportAsHtml(reportHtml, filename = 'chest-xray-report.html') {
    // === UPDATED Embedded Styles to match printPreparedHtml (ChestXray Single Column) ===
    const printStyles = `
        @page { size: A4; margin: 12mm; }
        ...
        /* ChestXray Findings Styles (Single Column) */
        /* === UPDATED SELECTOR === */
        .report-findings-section .cxr-findings-container { margin-top: 3mm !important; }
        .report-findings-section .cxr-findings-container h4 { font-size: 1.05em; font-weight: bold; color: #000; margin-top: 0.6em; margin-bottom: 0.3em; padding-bottom: 1px; border-bottom: 1px dotted #aaa; }
        .report-findings-section .cxr-findings-container h4:first-child { margin-top: 0;}
        .report-findings-section .cxr-findings-container h5 { font-size: 1em; font-weight: bold; color: #333; margin-top: 0.5em; margin-bottom: 0.2em; border-bottom: none; padding-bottom: 0; padding-left: 5mm;}
        .report-findings-section .cxr-findings-container ul { list-style: none; padding-left: 5mm; margin-top: 0.2em; margin-bottom: 0.8em; }
        .report-findings-section .cxr-findings-container li { margin-bottom: 0.3em; line-height: 1.4; }
        .report-findings-section .cxr-findings-container li strong { color: #000; margin-right: 4px; display: inline-block; min-width: 100px; }
        .report-findings-section .cxr-findings-container p { margin-left: 5mm; margin-top: 0.2em; margin-bottom: 0.6em; }
        .report-findings-section .cxr-findings-container > hr + h4, .report-findings-section .cxr-findings-container > h4:last-of-type { margin-top: 8mm; padding-top: 4mm; border-top: 1px solid #ccc; border-bottom: none; font-size: 1.05em; padding-left: 0;}
        /* General layout */
    ...
    // === UPDATED ===
    console.log(`[ReportCore - ChestXray] Report exported as ${filename}`);
}
...
export function initReportSystem() {
    ...
    for (const key in requiredElements) {
        if (!requiredElements[key]) {
            // === UPDATED ===
            console.error(`[ReportCore - ChestXray] Initialization failed: Element with ID matching '${key}' not found.`);
            showNotification(`Report UI element missing: ${key}. Check HTML.`, "error");
            return; // Stop initialization if UI is broken
        }
    }
    // === UPDATED ===
    console.log("[ReportCore - ChestXray] All required report UI elements found.");

    // --- Generate Report Button Listener ---
    generateBtn.addEventListener('click', async () => {
        // === UPDATED ===
        console.log("[ReportCore - ChestXray] Generate Report button clicked.");
        // Reset UI
        ...
        try {
            ...
            if (!currentPatientData.reportingDoctor) {
                // === UPDATED ===
                console.warn("[ReportCore - ChestXray] Reporting doctor name is empty.");
                // Optionally provide a default or prompt? For now, just warn.
            }

            // === UPDATED: Dynamically import and call CXR data collection ===
            const { collectChestXrayData } = await import('../organs/chestxray/chestxray-module.js'); // <-- CHANGE
            const organData = collectChestXrayData(); // <-- CHANGE
            if (!organData || typeof organData.formatReportSectionHtml !== 'function') {
                 // === UPDATED ===
                 throw new Error("Failed to collect or format Chest X-ray assessment data."); // <-- CHANGE
            }

            const images = getUploadedImageData(true); // Get image data
            const reportTitle = defaultReportTitleEN; // Use CXR default title
            generatedReportHtml = formatReportAsHtml(currentPatientData, organData, images, reportTitle); // <-- Uses CXR title
            // === UPDATED ===
            console.log("[ReportCore - ChestXray] Report HTML generated.");

            // Display Report & Show Buttons
            reportPreview.innerHTML = generatedReportHtml;
            // === UPDATED ===
            console.log("[ReportCore - ChestXray] Report preview updated.");
            ...
            if (getApiKey() && getTranslationPrompt()) {
                ...
                // === UPDATED ===
                console.warn("[ReportCore - ChestXray] Translate button hidden: API Key or Prompt missing.");
                showNotification("Translation disabled: Config missing.", "info");
            }
            // === UPDATED Notification ===
            showNotification("Chest X-ray report generated successfully!", "success"); // <-- CHANGED

        } catch (error) {
            // === UPDATED ===
            console.error("[ReportCore - ChestXray] Error during report generation:", error);
    ...
    translateBtn.addEventListener('click', async () => {
        // === UPDATED ===
        console.log("[ReportCore - ChestXray] Translate Report button clicked.");
        ...
        try {
             ...
             const findingsContainer = tempDiv.querySelector('.report-findings-section .cxr-findings-container'); // <-- CHANGE SELECTOR
             if (!findingsContainer) {
                 // === UPDATED ===
                throw new Error("Could not find the findings container (.cxr-findings-container) in the generated HTML."); // <-- CHANGE CLASS NAME
             }
             ...
             // === UPDATED ===
             console.error("[ReportCore - ChestXray] Error extracting text:", extractError);
        ...
        try {
            ...
            // === UPDATED ===
            console.error("[ReportCore - ChestXray] Translation failed:", error);
    ...
    printBtn.addEventListener('click', () => {
        if (generatedReportHtml) {
            // === UPDATED Title ===
            const title = `CXR Report - ${currentPatientData?.patientId || 'Unknown'} - ${currentPatientData?.examDate || ''}`;
            printPreparedHtml(generatedReportHtml, title); // Uses the updated function with CXR CSS
    ...
    printTranslationBtn.addEventListener('click', () => {
        if (currentTranslationMarkdown) {
             ...
             // === UPDATED Title ===
             const titleVI = `Báo cáo X quang Ngực - ${currentPatientData?.patientId || 'Unknown'} - ${currentPatientData?.examDate || ''}`;
             printPreparedHtml(vietnameseReportHtml, titleVI); // Use the same print function with CXR CSS
    ...
    exportBtn.addEventListener('click', () => {
         if (generatedReportHtml) {
             ...
             // === UPDATED Filename ===
             const filename = `CXR_Report_${patientId}_${examDate}.html`;
             exportReportAsHtml(generatedReportHtml, filename); // Uses updated embedded CSS
         } else { showNotification("Please generate the report first.", "info"); }
     });
...
// === UPDATED Log ===
console.log("report-core.js (ChestXray Version - v1 Single Column) loaded."); // <-- CHANGED